...
&emsp;&emsp;$R$为$n$目关系，$S$为$m$目关系，其中$t_r\isin R$,$t_s \isin S$,则$\overgroup{t_rt_s}$称为元组的连接。

&emsp;&emsp;$\overgroup{t_rt_s}$是一个$n+m$列的元组，前$n$个分量为$R$中的一个$n$元组，后$m$个分量为$S$中的一个$m$元组。
...
$$
    R \cup S = \{ t | t \isin R \lor t \isin S\}
$$
...
$$
    R \mathop{\bowtie} \limits_{A \varTheta B} S = \{ \overgroup{t_rt_s} | t_r \in R \land t_s \in S \land t_r[A] \varTheta t_s[B] \}
$$
...
$$
    R \mathop{\bowtie} \limits_{A = B} S = \{ \overgroup{t_rt_s} | t_r \in R \land t_s \in S \land t_r[A] = t_s[B] \}
$$
...
$$
    R \mathop{\bowtie}  S = \{ \overgroup{t_rt_s}[U-B] | t_r \in R \land t_s \in S \land t_r[B] = t_s[B] \}
$$
...
$$
    R \div S = \{ t_r[X] | t_r \in R \land \varPi_Y(S) \sube Y_x \}
$$
